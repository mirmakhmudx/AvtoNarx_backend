...
function makeListingData(array $overrides = []): ListingData
{
    return ListingData::fromArray(array_merge([
        'source_id' => 1,
...
it('updates price and creates a snapshot when price changes', function () {
    $this->service->ingest(makeListingData(['price_amount' => 145000000]));
    $this->service->ingest(makeListingData(['price_amount' => 150000000]));
...
it('rejects a "dogovornaya"-style listing is not applicable here (handled by MoneyExtractor upstream), but accepts valid price', function () {
    ...
    $listing = $this->service->ingest(makeListingData(['price_amount' => 150_000_000]));
