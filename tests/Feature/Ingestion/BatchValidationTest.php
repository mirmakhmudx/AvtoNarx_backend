...
function batchWithItem(array $itemOverrides = array(), array $removeKeys = array()): array
{
...
function postBatch(array $payload): \Illuminate\Testing\TestResponse
{
    return test()
        ->withHeaders(array('Idempotency-Key' => (string) Str::uuid()))
...
it('rejects an item with a MISSING year (TZ 8.2)', function () {
    postBatch(batchWithItem(array(), array('year')))
        ->assertStatus(422)
        ->assertJsonValidationErrors(array('items.0.year' => 'invalid_year'));
});

it('rejects an item with an out-of-range year', function () {
    postBatch(batchWithItem(array('year' => 1900)))
        ->assertStatus(422)
        ->assertJsonValidationErrors(array('items.0.year' => 'invalid_year'));
});

it('rejects an item with a MISSING content_hash (TZ 8.2)', function () {
    postBatch(batchWithItem(array(), array('content_hash')))
        ->assertStatus(422)
        ->assertJsonValidationErrors(array('items.0.content_hash' => 'invalid_content_hash'));
});

it('rejects an item with a non-hex content_hash', function () {
    postBatch(batchWithItem(array('content_hash' => str_repeat('z', 64))))
        ->assertStatus(422)
        ->assertJsonValidationErrors(array('items.0.content_hash' => 'invalid_content_hash'));
});

it('rejects an item whose URL is on a different domain than the source (invalid_url_domain, TZ 8.2)', function () {
    postBatch(batchWithItem(array('url' => 'https://evil.example.com/fake-car')))
        ->assertStatus(422)
        ->assertJsonValidationErrors(array('items.0.url' => 'invalid_url_domain'));
});
...
it('accepts a URL on a subdomain of the source domain', function () {
    ...
    postBatch(batchWithItem(array('url' => 'https://uz.olx.uz/d/obyavlenie/test.html')))
        ->assertStatus(202);
});
