...
function makeStaleTestListing(array $overrides = []): MarketListing
{
...
it('deactivates active listings whose last_seen_at is older than 72 hours', function () {
    $stale = makeStaleTestListing(['last_seen_at' => now()->subHours(73)]);
...
it('leaves recently-seen active listings untouched', function () {
    $fresh = makeStaleTestListing(['last_seen_at' => now()->subHours(10)]);
...
it('does not resurrect listings that are already inactive', function () {
    $alreadyInactive = makeStaleTestListing(['status' => 'inactive', 'last_seen_at' => now()->subHours(300)]);
...
it('preserves a listing right at the edge of the 72-hour window (not yet stale)', function () {
    $justInsideWindow = makeStaleTestListing(['last_seen_at' => now()->subHours(71)->subMinutes(59)]);
...
it('processes multiple listings across sources in a single run', function () {
    ...
    $staleA = makeStaleTestListing(['last_seen_at' => now()->subHours(100)]);
    $staleB = makeStaleTestListing(['source_id' => $otherSource->id, 'last_seen_at' => now()->subHours(200)]);
    $fresh = makeStaleTestListing(['last_seen_at' => now()->subHours(1)]);
