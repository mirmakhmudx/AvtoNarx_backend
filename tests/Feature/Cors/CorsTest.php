...
it('reflects a configured trusted origin on API responses (TZ 16)', function () {
    config()->set('cors.allowed_origins', array('https://trusted.example'));

    $this->withHeaders(array('Origin' => 'https://trusted.example'))
...
it('does NOT allow an untrusted origin (TZ 16)', function () {
    ...
    $response = $this->withHeaders(array('Origin' => 'https://evil.example'))
        ->getJson('/api/v1/brands');
...
it('handles a CORS preflight (OPTIONS) for a trusted origin', function () {
    ...
    $this->call('OPTIONS', '/api/v1/brands', array(), array(), array(), array(
